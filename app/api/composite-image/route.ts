...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Request params:", { superheroImageUrl, frameType, generationId, userId })
    ...
    try {
      ...
      return NextResponse.json({ error: "Failed to fetch superhero image from external source" }, { status: 500 })
    }

    if (!superheroResponse.ok) {
      console.error("Superhero image fetch failed:", superheroResponse.status, superheroResponse.statusText)
    ...
    try {
      ...
      return NextResponse.json({ error: "Failed to process superhero image" }, { status: 500 })
    }
    ...
    console.log("Composite positioning:", { offsetX, offsetY, superheroWidth, superheroHeight })
    ...
    try {
      ...
      return NextResponse.json({ error: "Failed to upload composite image to storage" }, { status: 500 })
    }
