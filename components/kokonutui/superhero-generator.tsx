...
import { Camera, Upload, Wand2, Download, RotateCcw, Loader2 } from "lucide-react"
  ...
  const generateSuperhero = useCallback(async () => {
    ...
    try {
      ...
      const pollForCompletion = async (predictionId: string) => {
        const checkResponse = await fetch(`/api/check-generation?id=${predictionId}`)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <CardContent className="space-y-4">
            ...
            {capturedImage && (
              <div className="space-y-4">
                ...
                <Button onClick={generateSuperhero} disabled={isGenerating} className="w-full gap-2">
                  {isGenerating ? (
                    <>
        ...
        <Card>
          ...
          <CardContent>
            ...
            {generationResult && (
              <div className="space-y-4">
                {generationResult.status === "processing" || generationResult.status === "starting" ? (
                  <div className="aspect-square bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center">
                    <div className="text-center">
                      <Loader2 className="h-12 w-12 mx-auto mb-2 animate-spin text-blue-500" />
                      <p className="text-gray-600 dark:text-gray-400">Creating your superhero...</p>
                    </div>
                  </div>
                ) : generationResult.status === "succeeded" && generationResult.output?.[0] ? (
                  <div className="space-y-4">
                    <div className="relative aspect-square bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden">
                      <Image
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-medium text-gray-900 dark:text-white">AI Generation</h4>
                <p className="text-gray-600 dark:text-gray-400">
                  Our AI transforms your photo into a Super Hero Masjid character
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
